...
lat_buildings = [
    ...
    51.541987   % 5 itsu
    51.541708   % 6 PRET
    51.541687   % 7 Bread Street Kitchena
];
lon_buildings = [
   -0.012031
   -0.013204
...
function [startName, goalName] = parse_dialogue_intent(sentence, candidateNames)
    ...
    for i = 1:numel(candidateNames)
        ...
        if ~isempty(pos)
            foundNames{end+1} = cand; 
            foundPos(end+1) = pos(1); 
        end
    end
...
function create_task3_gui(nodes, graph, nameToId, linkedLists, landmarkKD, signalKD, ...
    ...
    fig = uifigure('Name','Task 3 - Human Robot Interaction GUI', ...
        'Position',[80 80 1500 840]);


    fig.UserData.schedulerQueue = [];
    fig.UserData.serialCounter = 0;
    ...
    leftGrid.RowHeight = {22,22,22,22,22,22,22,22,22,22,22,28,28,28,28,28,28,120,'1x',30,30};
    ...
    uilabel(leftGrid,'Text','Task priority:');
    efPriority = uieditfield(leftGrid,'numeric','Value',3,'Limits',[1 10],'RoundFractionalValues','on');

    uilabel(leftGrid,'Text','Task name:');
    efTaskName = uieditfield(leftGrid,'text','Value','Custom Task');

    uilabel(leftGrid,'Text','Preset dialogue:');
    taDialogue = uitextarea(leftGrid, ...
        'Value', {'Guide me from W2 to Templar via S5 and S6'});
    taDialogue.Layout.Row = [11 18];
    taDialogue.Layout.Column = 2;

    btnRun = uibutton(leftGrid,'Text','Run Query');
    btnRun.Layout.Row = 19;
    btnRun.Layout.Column = [1 2];

    btnAddTask = uibutton(leftGrid,'Text','Add Current Query to Scheduler');
    btnAddTask.Layout.Row = 20;
    btnAddTask.Layout.Column = [1 2];

    btnClearQueue = uibutton(leftGrid,'Text','Clear Scheduler Queue');
    btnClearQueue.Layout.Row = 21;
    btnClearQueue.Layout.Column = [1 2];

    rightPanel = uipanel(gl,'Title','Result & Map');
    rightGrid = uigridlayout(rightPanel,[3 1]);
    rightGrid.RowHeight = {230,140,'1x'};

    taResult = uitextarea(rightGrid,'Editable','off');
    taQueue  = uitextarea(rightGrid,'Editable','off');
    ax = uiaxes(rightGrid);
    title(ax,'Map View');
    xlabel(ax,'X (m)');
    ylabel(ax,'Y (m)');

    taQueue.Value = {'Scheduler queue is empty.'};

    gui_plot_base_map(ax, occupancyGrid, nodes, graph, mapWidthMeters, mapHeightMeters);

    btnAddTask.ButtonPushedFcn = @(src,event) gui_add_task_to_queue( ...
        fig, ddType, ddStart, ddGoal, ddPoint, efK, efSignals, cbReturn, efPriority, efTaskName, ...
        taResult, taQueue, nodes);

    btnClearQueue.ButtonPushedFcn = @(src,event) gui_clear_scheduler_queue(fig, taResult, taQueue);
    ...
    btnRun.ButtonPushedFcn = @(src,event) local_run_router( ...
        fig, ddType, ddStart, ddGoal, ddPoint, efK, efSignals, cbReturn, taDialogue, ...
        taResult, taQueue, ax, nodes, graph, nameToId, linkedLists, landmarkKD, signalKD, ...
        occupancyGrid, cellSize, mapWidthMeters, mapHeightMeters);
end

function local_run_router(fig, ddType, ddStart, ddGoal, ddPoint, efK, efSignals, cbReturn, taDialogue, ...
    taResult, taQueue, ax, nodes, graph, nameToId, linkedLists, landmarkKD, signalKD, ...
    occupancyGrid, cellSize, mapWidthMeters, mapHeightMeters)

    if strcmp(ddType.Value, 'scheduler_demo')
        gui_run_custom_scheduler(fig, taResult, taQueue, ax, nodes, graph, nameToId, ...
            linkedLists, landmarkKD, signalKD, occupancyGrid, cellSize, ...
            mapWidthMeters, mapHeightMeters);
    else
        gui_run_query(ddType, ddStart, ddGoal, ddPoint, efK, efSignals, cbReturn, taDialogue, ...
    ...
    try
        ...
        switch q.type
            ...
            case 'speculative_dialogue_demo'
                ...
                traceLines = build_nlp_trace(latestText, nodes, ddStart.Value);
        ...
        if strcmp(q.type, 'speculative_dialogue_demo')
            traceLines = build_nlp_trace(get_latest_nonempty_line(taDialogue.Value), nodes, ddStart.Value);
...
function gui_live_nlp_update(ddType, ddStart, taDialogue, taResult, ax, ...
    ...
    try
        ...
        if isempty(strtrim(latestText))
            taResult.Value = { ...
                'Live NLP mode: type a navigation sentence in the text box.', ...
                ' ', ...
                'NLP interpretation trace:', ...
                'No input yet.'};
            gui_plot_base_map(ax, occupancyGrid, nodes, graph, mapWidthMeters, mapHeightMeters);
            return
        end
        ...
        taResult.Value = [ ...
            ...
            {' '}; ...
            {'--------------------------------'}; ...
            build_nlp_trace(get_latest_nonempty_line(taDialogue.Value), nodes, ddStart.Value).'];
        gui_plot_base_map(ax, occupancyGrid, nodes, graph, mapWidthMeters, mapHeightMeters);
    end
end
...
function traceLines = build_nlp_trace(userText, nodes, defaultStartName)
    words = strsplit(strtrim(userText));
    traceLines = {};

    if isempty(words) || (numel(words) == 1 && isempty(words{1}))
        traceLines = {'NLP interpretation trace:', 'No input yet.'};
        return
    end

    traceLines{end+1} = 'NLP interpretation trace:';
    traceLines{end+1} = ' ';

    for i = 1:numel(words)
        partialText = strjoin(words(1:i), ' ');
        [q, isReady, msg] = parse_natural_language_live(partialText, nodes, defaultStartName);

        traceLines{end+1} = ['Step ' num2str(i) ': ' partialText];

        if isfield(q, 'type') && ~isempty(q.type)
            traceLines{end+1} = ['  Intent: ' q.type];
        else
            traceLines{end+1} = '  Intent: <unknown>';
        end

        if isfield(q, 'start') && ~isempty(q.start)
            traceLines{end+1} = ['  Start: ' q.start];
        elseif isfield(q, 'startWait') && ~isempty(q.startWait)
            traceLines{end+1} = ['  Start: ' q.startWait];
        else
            traceLines{end+1} = '  Start: <unknown>';
        end

        if isfield(q, 'goal') && ~isempty(q.goal)
            traceLines{end+1} = ['  Goal: ' q.goal];
        elseif isfield(q, 'pointName') && ~isempty(q.pointName)
            traceLines{end+1} = ['  Reference point: ' q.pointName];
        else
            traceLines{end+1} = '  Goal/Reference: <unknown>';
        end

        if isfield(q, 'requiredSignals') && ~isempty(q.requiredSignals)
            traceLines{end+1} = ['  Signals: ' strjoin(q.requiredSignals, ', ')];
        end

        if isfield(q, 'k') && ~isempty(q.k)
            traceLines{end+1} = ['  k: ' num2str(q.k)];
        end

        if isReady
            traceLines{end+1} = '  Status: valid query';
        else
            traceLines{end+1} = ['  Status: ' msg];
        end

        traceLines{end+1} = ' ';
    end
end
function q = build_query_from_gui(ddType, ddStart, ddGoal, ddPoint, efK, efSignals, cbReturn)
    q = struct();
    q.type = ddType.Value;
    ...
    switch q.type
        ...
        otherwise
            error('Current query type cannot be directly added as a scheduler task.');
    end
end
function gui_add_task_to_queue(fig, ddType, ddStart, ddGoal, ddPoint, efK, efSignals, cbReturn, efPriority, efTaskName, ...
    taResult, taQueue, nodes)

    try
        taResult.Value = {'Add button clicked. Processing task...'};

        if strcmp(ddType.Value, 'scheduler_demo') || strcmp(ddType.Value, 'speculative_dialogue_demo')
            taResult.Value = {'Please choose a concrete query type before adding a task to the scheduler.'};
            return
        end

        q = build_query_from_gui(ddType, ddStart, ddGoal, ddPoint, efK, efSignals, cbReturn);

        priority = max(1, round(efPriority.Value));

        taskNameRaw = efTaskName.Value;
        if isstring(taskNameRaw)
            taskName = strtrim(char(taskNameRaw));
        else
            taskName = strtrim(taskNameRaw);
        end

        if isempty(taskName)
            taskName = ['Task-' q.type];
        end

        if ~isfield(fig.UserData, 'schedulerQueue') || isempty(fig.UserData.schedulerQueue)
            queue = [];
        else
            queue = fig.UserData.schedulerQueue;
        end

        if ~isfield(fig.UserData, 'serialCounter') || isempty(fig.UserData.serialCounter)
            fig.UserData.serialCounter = 0;
        end

        fig.UserData.serialCounter = fig.UserData.serialCounter + 1;
        task = make_task(taskName, priority, fig.UserData.serialCounter, q);

        queue = scheduler_push(queue, task);
        fig.UserData.schedulerQueue = queue;

        taQueue.Value = format_scheduler_queue(queue);

        taResult.Value = { ...
            'Task added to scheduler queue.', ...
            ['Task name: ' taskName], ...
            ['Task type: ' q.type], ...
        ...
        taResult.Value = { ...
            'Error while adding task to scheduler:', ...
            ME.message};


        disp('===== gui_add_task_to_queue ERROR =====');
        disp(getReport(ME, 'extended', 'hyperlinks', 'off'));
    end
end
function gui_run_custom_scheduler(fig, taResult, taQueue, ax, nodes, graph, nameToId, ...
    linkedLists, landmarkKD, signalKD, occupancyGrid, cellSize, mapWidthMeters, mapHeightMeters)

    try
        queue = fig.UserData.schedulerQueue;

        if isempty(queue)
            taResult.Value = {'Scheduler queue is empty. Add tasks first.'};
            gui_plot_base_map(ax, occupancyGrid, nodes, graph, mapWidthMeters, mapHeightMeters);
            return
        end

        processedLines = {'Custom scheduler execution order:', ' '};
        lastResult = [];
        lastQ = [];
        ...
        while ~isempty(queue)
            ...
            processedLines{end+1} = sprintf('[Priority %d] %s -> %s', ...
                task.priority, task.name, result.summary); 
        ...
        fig.UserData.schedulerQueue = [];
        taQueue.Value = {'Scheduler queue is empty.'};
        taResult.Value = processedLines(:);


        gui_plot_base_map(ax, occupancyGrid, nodes, graph, mapWidthMeters, mapHeightMeters);
        if ~isempty(lastResult) && ~isempty(lastQ)
            gui_plot_query_result(ax, lastQ, lastResult, nodes, graph, nameToId, ...
                occupancyGrid, cellSize, mapWidthMeters, mapHeightMeters);
        end

    catch ME
        taResult.Value = {['Error while running scheduler: ' ME.message]};
        gui_plot_base_map(ax, occupancyGrid, nodes, graph, mapWidthMeters, mapHeightMeters);
    end
end
function gui_clear_scheduler_queue(fig, taResult, taQueue)
    fig.UserData.schedulerQueue = [];
    fig.UserData.serialCounter = 0;
    taQueue.Value = {'Scheduler queue is empty.'};
    taResult.Value = {'Scheduler queue cleared.'};
end
function outLines = format_scheduler_queue(queue)
    if isempty(queue)
        outLines = {'Scheduler queue is empty.'};
        return
    end

    outLines = {'Current scheduler queue:', ' '};
    for i = 1:numel(queue)
        q = queue(i).payload;
        line = sprintf('%d) [P%d] %s | type=%s', ...
            i, queue(i).priority, queue(i).name, q.type);

        if isfield(q, 'start') && isfield(q, 'goal')
            line = sprintf('%s | %s -> %s', line, q.start, q.goal);
        elseif isfield(q, 'startWait')
            line = sprintf('%s | start=%s', line, q.startWait);
        elseif isfield(q, 'pointName')
            line = sprintf('%s | point=%s', line, q.pointName);
        end
